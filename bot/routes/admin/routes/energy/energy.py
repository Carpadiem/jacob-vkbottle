...
async def energy_get(m: Message, pid=None):
    ...
    if not is_number(pid):
        ...
        return
    # get recipient bank
    recipient_energy: EnergyEntity = energyRepo.find_one_by({ 'player_id': int(pid) })
...
async def energy_set(m: Message, pid=None, value=None):
    ...
    # updates
    # energy set
    energyRepo.update({ 'player_id': int(pid) }, { 'energy': int(value) })
    energyRepo.update({ 'player_id': int(pid) }, { 'ts_previous_use': ts_now() })
