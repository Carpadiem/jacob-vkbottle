...
async def show_my_business(m: Message):
    # entities
    player: PlayerEntity = playerRepo.find_one_by({ 'user_id': m.from_id })
    business: BusinessEntity = businessRepo.find_one_by({ 'user_id': m.from_id })
...
async def buy_business(m: Message):
    ...
    if player.money < purchased_business_price:
        ...
        return
    # reduce money
    playerRepo.update({ 'user_id': m.from_id }, { 'money': player.money - purchased_business_price })
    # update business repo (business_id=purchased_business_id, ts_previous_profit=ts_now)
    businessRepo.update({ 'user_id': m.from_id }, { 'business_id': purchased_business_id })
    businessRepo.update({ 'user_id': m.from_id }, { 'ts_previous_profit': ts_now() })
    # answer
...
async def business_profit(m: Message):
    ...
    playerRepo.update({ 'user_id': m.from_id }, { 'money': player.money + result_profit }) # update player's money
    businessRepo.update({ 'user_id': m.from_id }, { 'ts_previous_profit': ts_now() }) # update player's business profit ts_previous_profit
    # answer
...
async def business_sell(m: Message):
    ...
    businessRepo.update({ 'user_id': m.from_id }, { 'business_id': 0 })
    # update player's money (set money = money + business_price_for_sell)
    playerRepo.update({ 'user_id': m.from_id }, { 'money': player.money + business_price_for_sell })
